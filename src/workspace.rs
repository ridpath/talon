...
impl WorkspaceManager {
    ...
    pub fn add(workspace_name: &str, challenge_name: &str) -> Result<(), String> {
        ...
        workspace.challenges.insert(challenge_name.to_string(), challenge);
        Self::save_workspace(&workspace)?;
        
        let challenge_dir = Self::get_workspace_dir(workspace_name)?
            .join(challenge_name);
        fs::create_dir_all(&challenge_dir)
            .map_err(|e| format!("Failed to create challenge directory: {}", e))?;
        
        println!("Challenge '{}' added to workspace '{}'", challenge_name, workspace_name);
    ...
    pub fn set_status(workspace_name: &str, challenge_name: &str, status: &str) -> Result<(), String> {
        let mut workspace = Self::load_workspace(workspace_name)?;
        ...
        if let Some(challenge) = workspace.challenges.get_mut(challenge_name) {
            ...
            Err(format!("Challenge '{}' not found in workspace", challenge_name))
        }
    }
    ...
    pub fn add_solution(workspace_name: &str, challenge_name: &str, solution_path: &str) -> Result<(), String> {
        let mut workspace = Self::load_workspace(workspace_name)?;
                    ...
                    if entry.path().is_dir() {
                        if let Some(name) = entry.file_name().to_str() {
                            if let Ok(workspace) = Self::load_workspace(name) {
                                let solved = workspace.challenges.values()
