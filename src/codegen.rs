...
use crate::ast::{
    Command, Expr, Literal, Control, CryptoCommand, BlockchainCommand,
    OffensiveCommand, TypeHint, TypedVar, TryCatch,
};
...
pub fn build_script(commands: &[Command], static_build: bool) -> Result<(), Box<dyn std::error::Error>> {
...
fn get_required_crates(cmd: &Command) -> Vec<&'static str> {
    match cmd {
        Command::Crypto(crypto) => match crypto {
            CryptoCommand::GenerateECCKeypair { .. }
            | CryptoCommand::ECDSASign { .. } => vec!["openssl", "hex"],
        ...
        Command::Blockchain(bc) => match bc {
            ...
            BlockchainCommand::EVMDisassemble { .. }
            | BlockchainCommand::ScanReentrancy { .. } => vec!["hex"],
            _ => vec![],
        },
        Command::Offensive(off) => match off {
            OffensiveCommand::AssembleSyscall { .. }
            | OffensiveCommand::BuildShellcode { .. } => vec!["keystone", "capstone"],
            OffensiveCommand::BridgeIDA { .. }
            | OffensiveCommand::BridgeGhidra { .. } => vec!["serde", "serde_json"],
        ...
        | Command::DecodeBase64 { .. } => vec!["base64"],
...
fn generate_main_rs(commands: &[Command], crates: &HashSet<&str>) -> Result<String, std::fmt::Error> {
    ...
    if is_async {
        writeln!(code, "\n#[tokio::main]")?;
        writeln!(code, "async fn main() -> Result<(), Box<dyn std::error::Error>> {{")?;
    } else {
        writeln!(code, "\nfn main() {{")?;
    }

    writeln!(code, "    let mut vars: HashMap<String, String> = HashMap::new```rust
    new();
")?;
    for cmd in commands {
...
fn generate_command(buf: &mut String, cmd: &Command, is_async: bool) -> Result<(), std::fmt::Error> {
    match cmd {
        Command::RunCommand { command } => {
            writeln!(buf, "    println!(\"[RUN] {}\");", command)?;
            writeln!(buf, "    let out = SysCommand::new(\"sh\").arg(\"-c\").arg(\"{}\").output().expect(\"Exec fail\");", command)?;
            writeln!(buf, "    println!(\"{{}}\", String::from_utf8_lossy(&out.stdout));")?;
        }
        Command::TypedDecl(TypedVar { name, var_type, value }) => {
            ...
            writeln!(buf, "    let {}: {} = {};", name, rust_type, generate_expr(value))?;
        }
        Command::Download { url, path } => {
            if is_async {
                writeln!(buf, "    let data = reqwest::Client::new().get(\"{}\").send().await?.bytes().await?;", url)?;
            } else {
                writeln!(buf, "    let data = reqwest::blocking::Client::new().get(\"{}\").send()?.bytes()?;", url)?;
            }
            writeln!(buf, "    fs::write(\"{}\", data).expect(\"Failed to write file\");", path)?;
        }
        Command::Beacon { url, interval } => {
            if is_async {
                ...
                writeln!(buf, "        loop {{")?;
                writeln!(buf, "            let _ = client.get(\"{}\").send().await;", url)?;
                writeln!(buf, "            time::sleep(Duration::from_secs({})).await;", interval)?;
                writeln!(buf, "        }}")?;
                writeln!(buf, "    }});")?;
            } else {
                writeln!(buf, "    std::thread::spawn(move || {{")?;
                writeln!(buf, "        let client = reqwest::blocking::Client::new();")?;
                writeln!(buf, "        loop {{")?;
                writeln!(buf, "            let _ = client.get(\"{}\").send();", url)?;
                writeln!(buf, "            std::thread::sleep(Duration::from_secs({}));", interval)?;
                writeln!(buf, "        }}")?;
                writeln!(buf, "    }});")?;
            }
        }
        Command::WriteFile { data, path } => {
            writeln!(buf, "    fs::write(\"{}\", {}).expect(\"Write fail\");", path, generate_expr(data))?;
        }
        Command::ReadFile { path, var } => {
            writeln!(buf, "    let data = fs::read_to_string(\"{}\").expect(\"Read fail\");", path)?;
        ...
        Command::Expr(expr) => {
            writeln!(buf, "    println!(\"[EXPR] {{}}\", {});", generate_expr(expr))?;
        }
        ...
        Command::CallFunction { name, args } => {
            ...
            let call_args = (0..args.len()).map(|i| format!("arg{}", i)).collect::<Vec<_>>().join(", ");
            writeln!(buf, "    {}({});", name, call_args)?;
        }
        Command::ToolExec { tool, args } => {
            let joined_args = args.iter().map(generate_expr).collect::<Vec<_>>().join(", ");
            writeln!(buf, "    let output = SysCommand::new(\"{}\").args(vec![{}]).output().expect(\"Tool exec failed\");", tool, joined_args)?;
            writeln!(buf, "    println!(\"[TOOL] {{}}\", String::from_utf8_lossy(&output.stdout));")?;
        }
        Command::TryCatch(TryCatch { try_body, catch_var, catch_body }) => {
            writeln!(buf, "    let result: Result<(), Box<dyn std::error::Error>> = (|| {{")?;
...
fn generate_expr(expr: &Expr) -> String {
    match expr {
        ...
        Expr::Literal(Literal::ByteArray(s)) => format!("hex::decode(\"{}\").expect(\"Invalid hex string\")", s),
        Expr::Ident(id) => format!("vars.get(\"{}\").cloned().unwrap_or_default()", id),
        Expr::BinaryOp { op, left, right } => format!("({} {} {})", generate_expr(left), op, generate_expr(right)),
        Expr::RegexMatch { regex, haystack } => {
            let safe_regex = regex.replace('\\', "\\\\").replace('"', "\\\"");
            format!("Regex::new(r\"{}\").expect(\"Invalid regex\").is_match(&{})", safe_regex, generate_expr(haystack))
        }
        Expr::InterpolatedString(parts) => {
            let joined = parts.iter().map(generate_expr).collect::<Vec<_>>().join(" + ");
            format!("({})", joined)
        }
