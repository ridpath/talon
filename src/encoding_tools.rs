...
impl HTMLEncoder {
    ...
    pub fn encode_decimal(data: &str) -> String {
        data.chars().map(|c| format!("&#{};", c as u32)).collect()
    }
...
impl BinaryConverter {
    ...
    pub fn from_octal(octal: &str) -> Result<Vec<u8>, String> {
        octal
            .split_whitespace()
            .map(|o| u8::from_str_radix(o, 8).map_err(|e| format!("Invalid octal: {}", e)))
            .collect()
    }
}
...
impl SubstitutionCipher {
    ...
    pub fn encode(&self, text: &str) -> String {
        let map: HashMap<char, char> = self.alphabet.chars().zip(self.key.chars()).collect();
    ...
    pub fn decode(&self, text: &str) -> String {
        let map: HashMap<char, char> = self.key.chars().zip(self.alphabet.chars()).collect();
