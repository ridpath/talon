...
impl CVEScanner {
    ...
    fn init_local_database(&mut self) {
        ...
        self.cve_db.insert("CVE-2021-3156".to_string(), CVEInfo {
            ...
            affected_versions: vec!["1.8.2".to_string(), "1.8.31p2".to_string(), "1.9.0".to_string(), "1.9.5p1".to_string()],
            ...
            vulnerable_functions: vec!["set_cmnd".to_string(), "sudo_ldap_role_to_priv".to_string()],
            patch_indicators: vec!["size_t len = strlen(info->command) + 1;".to_string()],
        });
        
        self.cve_db.insert("CVE-2022-0847".to_string(), CVEInfo {
            cve_id: "CVE-2022-0847".to_string(),
            title: "Dirty Pipe - Arbitrary File Overwrite".to_string(),
            description: "Linux kernel pipe buffer vulnerability allowing arbitrary file overwrite".to_string(),
            affected_versions: vec!["5.8".to_string(), "5.16.11".to_string()],
            patched_versions: vec!["5.16.12".to_string(), "5.15.26".to_string(), "5.10.102".to_string()],
            cvss_score: 7.8,
            exploit_available: true,
            ...
            vulnerable_functions: vec!["pipe_write".to_string(), "copy_page_to_iter_pipe".to_string()],
            patch_indicators: vec!["buf->flags = 0;".to_string()],
        });
        
        self.cve_db.insert("CVE-2023-32233".to_string(), CVEInfo {
            cve_id: "CVE-2023-32233".to_string(),
            title: "Netfilter nf_tables UAF".to_string(),
            description: "Use-after-free in Netfilter nf_tables allows local privilege escalation".to_string(),
            ...
            attack_vector: AttackVector::Local,
            references: vec![
                "https://www.exploit-db.com/exploits/51542".to_string(),
            ],
            vulnerable_functions: vec!["nf_tables_commit".to_string(), "nft_chain_validate".to_string()],
        ...
        self.cve_db.insert("CVE-2023-2640".to_string(), CVEInfo {
            cve_id: "CVE-2023-2640".to_string(),
            title: "GameOver(lay) - Ubuntu OverlayFS Privilege Escalation".to_string(),
            description: "OverlayFS vulnerability in Ubuntu kernels allowing privilege escalation".to_string(),
        ...
        self.cve_db.insert("CVE-2024-1086".to_string(), CVEInfo {
            ...
            attack_vector: AttackVector::Local,
            references: vec![
                "https://github.com/Notselwyn/CVE-2024-1086".to_string(),
            ],
            vulnerable_functions: vec!["nft_verdict_init".to_string()],
            patch_indicators: vec!["nft_chain_validate_dependency".to_string()],
        });
        
        self.cve_db.insert("CVE-2022-2586".to_string(), CVEInfo {
            cve_id: "CVE-2022-2586".to_string(),
            title: "Netfilter nf_tables UAF (2022)".to_string(),
            description: "Use-after-free vulnerability in nf_tables cross-table reference handling".to_string(),
        ...
        self.cve_db.insert("CVE-2023-0179".to_string(), CVEInfo {
            ...
            attack_vector: AttackVector::Local,
            references: vec![
                "https://github.com/TurtleARM/CVE-2023-0179-PoC".to_string(),
            ],
            vulnerable_functions: vec!["nfnl_osf_add_callback".to_string()],
            patch_indicators: vec!["nfnl_osf_remove_callback".to_string()],
        });
        
        println!("[CVE] [OK] Loaded {} CVEs into local database", self.cve_db.len());
    }
    ...
    pub fn scan_target(&self, target: &str, cve_list: &[String], suggest_exploit: bool, generate_poc: bool) -> Result<CVEScanResult, String> {
        ...
        for cve_id in cve_list {
            ...
            if let Some(cve_info) = self.cve_db.get(cve_id) {
                let status = self.check_vulnerability(target, cve_info, suggest_exploit, generate_poc)?;
                
                if status.is_vulnerable {
                    vulnerable_count += 1;
                    println!("[SCAN] WARNING: VULNERABLE: {} (confidence: {:.1}%)", cve_id, status.confidence);
                    ...
                    println!("[SCAN] [OK] Not vulnerable: {} (confidence: {:.1}%)", cve_id, status.confidence);
                }
                
                vulnerabilities.push(status);
            } else {
                println!("[SCAN] CVE not in database: {} - using heuristic scan", cve_id);
                vulnerabilities.push(self.heuristic_check(target, cve_id)?);
            }
        }
        
        let risk_score = self.calculate_risk_score(vulnerable_count, exploitable_count, &vulnerabilities);
    ...
    fn check_vulnerability(&self, target: &str, cve_info: &CVEInfo, suggest_exploit: bool, generate_poc: bool) -> Result<VulnerabilityStatus, String> {
        let mut evidence = Vec::new();
        let mut confidence: f64 = 0.0;
        
        let version = self.detect_version(target)?;
        let is_vulnerable = self.is_version_vulnerable(&version, &cve_info.affected_versions, &cve_info.patched_versions);
    ...
    fn is_version_vulnerable(&self, detected: &Option<String>, affected: &[String], patched: &[String]) -> bool {
        if let Some(ref version) = detected {
            ...
            for affected_ver in affected {
                let affected_parts: Vec<u32> = affected_ver.split('.').filter_map(|s| s.parse().ok()).collect();
                if ver_parts >= affected_parts {
                    for patch_ver in patched {
                        let patch_parts: Vec<u32> = patch_ver.split('.').filter_map(|s| s.parse().ok()).collect();
    ...
    fn check_symbols(&self, target: &str, vulnerable_functions: &[String]) -> Result<Vec<String>, String> {
    ...
    fn generate_poc_code(&self, cve_info: &CVEInfo) -> Result<String, String> {
        ...
        let poc = format!(r#"#!/usr/bin/env python3
...
"#,
    ...
    fn calculate_risk_score(&self, vulnerable_count: usize, exploitable_count: usize, vulnerabilities: &[VulnerabilityStatus]) -> f64 {
        ...
        let confidence_avg: f64 = vulnerabilities.iter()
    ...
    fn generate_recommendations(&self, vulnerabilities: &[VulnerabilityStatus]) -> Vec<String> {
        ...
        for vuln in vulnerabilities {
            if vuln.is_vulnerable {
                recommendations.push(format!("[{}] Update to patched version immediately", vuln.cve_id));
                
                if let Some(ref exploit) = vuln.suggested_exploit {
                    recommendations.push(format!("[{}] Exploit available: {} - High priority fix", vuln.cve_id, exploit));
                }
            }
        }
        
        if recommendations.is_empty() {
            recommendations.push("No immediate vulnerabilities detected - continue regular security updates".to_string());
        }
        ...
        for vuln in &result.vulnerabilities_found {
            if vuln.poc_generated {
                if let Some(ref poc) = vuln.poc_code {
                    let poc_filename = format!("poc_{}.py", vuln.cve_id.replace("-", "_").to_lowercase());
