...
impl QuickPwn {
    ...
    pub fn remote(host: &str, port: u16, binary: &str) -> Self {
        log::info!("Creating quick pwn context for {}:{} ({})", host, port, binary);
    ...
    pub fn symbol(&self, libc_name: &str, symbol: &str) -> Result<u64, String> {
        ...
        self.libc_db.resolve_address(libc_name, base, symbol)
    ...
    pub fn one_gadgets(&self, libc_name: &str) -> Result<Vec<u64>, String> {
        let base = self.libc_base.ok_or("Libc base not leaked")?;
        self.libc_db.get_one_gadgets(libc_name, base)
            .ok_or("One-gadgets not found".to_string())
    }
    
    /// Generate heap exploit
    pub fn heap_exploit(&self, technique: HeapTechnique, target: HeapTarget) -> Result<Vec<u8>, String> {
        let glibc = self.glibc_version.as_ref()
            .ok_or("Glibc version not set")?;
...
mod tests {
    ...
    fn test_groom_heap() {
        let pwn = QuickPwn::remote("127.0.0.1", 9001, "./vuln");
        let script = pwn.groom_heap(GroomingStrategy::Spray { size: 0x80, count: 100 });
