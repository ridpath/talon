...
impl ReportGenerator {
    ...
    pub fn generate_markdown(&self, report: &ExploitReport) -> Result<String, String> {
        ...
        md.push_str(&format!("A **{}** vulnerability was identified in `{}` that allows for {}.\n\n", 
        ...
        md.push_str(&format!("- **Exploitability Score**: {:.2}/1.00\n\n", report.vulnerability.exploitability));
...
"#);
    ...
    pub fn save_report(&self, report: &ExploitReport, path: &str, format: ReportFormat) -> Result<(), String> {
        let content = match format {
            ReportFormat::Markdown => self.generate_markdown(report)?,
            ReportFormat::Html => self.generate_html(report)?,
            ReportFormat::Pdf => {
                return Err("PDF generation requires additional dependencies (not yet implemented)".to_string());
            }
        };
