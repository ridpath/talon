...
impl ROPGadgetFinder {
    ...
    pub fn analyze_file(&mut self, path: &str) -> Result<(), String> {
        ...
        if data.len() < 100 {
            eprintln!(
                "{} Binary is very small ({} bytes), may not contain useful gadgets",
                "[WARNING]".yellow(),
                data.len()
            );
        }
    ...
    pub fn analyze_bytes(&mut self, data: &[u8], base_addr: u64) -> Result<(), String> {
        ...
        eprintln!(
            "{} Found {} gadgets",
            "[OK]".green(),
            self.gadgets.len().to_string().cyan()
        );
    ...
    fn is_duplicate(&self, new_gadget: &Gadget) -> bool {
        self.gadgets
            .iter()
            .any(|g| g.address == new_gadget.address || g.instructions == new_gadget.instructions)
    }
    ...
    pub fn build_rop_chain(&self, target: ROPTarget) -> Result<Vec<u64>, String> {
        match target {
            ...
            ROPTarget::Mprotect { addr, size, prot } => self.build_mprotect_chain(addr, size, prot),
        }
    }
...
pub enum ROPTarget {
    Execve { binsh_addr: u64, libc_base: u64 },
    System { binsh_addr: u64, system_addr: u64 },
    Mprotect { addr: u64, size: u64, prot: u64 },
}
...
mod tests {
    ...
    fn test_categorization() {
        ...
        assert_eq!(
            finder.categorize_gadget(&syscall_gadget),
