...
use crate::formatter;
use crate::linter;
use crate::ai_suggestion::{SuggestionEngine, suggest_exploits_for_binary};
...
pub fn run(args: Vec<String>) {
    ...
    match args.iter().map(|s| s.as_str()).collect::<Vec<_>>().as_slice() {
        [_, "build", file] => {
            ...
            println!("{}", "[BUILD] Binary: ./talon_build/target/release/talon_script".green());
        ...
        [_, "analyze", "--binary", file] => {
            ...
            for c in cmds {
                if let crate::ast::Command::Reverse(ref r) = c {
                    crate::re_tools::handle_re_command(r).unwrap_or_else(|e| eprintln!("Error: {}", e));
                }
            }
        }
        ...
        [_, "examples", "copy", name] => {
            let lib = ExampleLibrary::new();
            lib.copy(name, None).unwrap_or_else(|e| eprintln!("Error: {}", e));
        }
        
        [_, "examples", "copy", name, dest] => {
            let lib = ExampleLibrary::new();
            lib.copy(name, Some(dest)).unwrap_or_else(|e| eprintln!("Error: {}", e));
        }
        ...
        [_, "workspace", "add", workspace, challenge] => {
            WorkspaceManager::add(workspace, challenge).unwrap_or_else(|e| eprintln!("Error: {}", e));
        }
...
"#);
        ...
        [_, "db", "search", query] => {
            ...
            println!("\n{} {} result(s) for '{}'", "[DB]".blue(), results.len(), query);
        ...
        [_, "db", "type", exploit_type] => {
            ...
            println!("\n{} {} exploit(s) of type '{}'", "[DB]".blue(), results.len(), exploit_type);
            for exploit in results {
        ...
        [_, "db", "platform", platform] => {
            ...
            println!("\n{} {} exploit(s) for '{}'", "[DB]".blue(), results.len(), platform);
            for exploit in results {
                println!("  {} - {}", exploit.cve_id.yellow(), exploit.title);
            }
        }

        [_, "analyze", binary_path] => {
            match TargetDetector::analyze(binary_path) {
        ...
        [_, "diff", file1, file2] => {
            match EnhancedBinaryDiffer::diff(file1, file2) {
        ...
        [_, "suggest", binary_path] => {
            println!("\n{} Analyzing binary and suggesting exploits...", "[AI]".cyan().bold());
            match suggest_exploits_for_binary(binary_path) {
                Ok(suggestions) => {
                    println!("\n{} Found {} potential exploit(s):\n", "[SUCCESS]".green().bold(), suggestions.len());
                    for suggestion in suggestions {
                        ...
                        println!("   Success Probability: {}%", (suggestion.success_probability * 100.0).to_string().green());
        ...
        [_, "suggest", binary_path, "--generate", rank] => {
            ...
            println!("\n{} Generating exploit code for suggestion #{}...", "[AI]".cyan().bold(), rank_num);
            match suggest_exploits_for_binary(binary_path) {
                Ok(suggestions) => {
                    if let Some(suggestion) = suggestions.get(rank_num - 1) {
                        ...
                        eprintln!("{} Suggestion #{} not found", "[ERROR]".red().bold(), rank_num);
                    }
                },
                Err(e) => eprintln!("{} {}", "[ERROR]".red().bold(), e),
            }
        ...
        [_, "config", "edit"] => {
            if let Some(path) = TalonConfig::get_config_path() {
                ...
                std::process::Command::new("notepad").arg(path).status().ok();
                ...
                eprintln!("{} Could not determine config file location", "[ERROR]".red());
            }
        }

        [_, "man", "--generate"] => {
            match ManPages::generate_all() {
        ...
        [_, "completion", shell] => {
            match CompletionGenerator::install(shell) {
        ...
        [_, "replay", "play", file] => {
            match TalonReplay::load(file) {
                Ok(replay) => replay.play().unwrap_or_else(|e| eprintln!("{} {}", "[ERROR]".red(), e)),
                Err(e) => eprintln!("{} {}", "[ERROR]".red(), e),
            }
        }

        [_, "replay", "export", file, output] => {
            match TalonReplay::load(file) {
                Ok(replay) => replay.export_to_talon_script(output).unwrap_or_else(|e| eprintln!("{} {}", "[ERROR]".red(), e)),
                Err(e) => eprintln!("{} {}", "[ERROR]".red(), e),
            }
        }
        ...
        [_, "tutorial", "first-blood"] | [_, "tutorial"] => {
            ...
            tutorial.start_first_blood().unwrap_or_else(|e| eprintln!("{} {}", "[ERROR]".red(), e));
        }
            ...
            match ChallengeMarketplace::browse() {
                Ok(challenges) => {
                    ...
                    for challenge in challenges {
                        ...
                        println!("  Author: {} | Rating: {}/5.0 | Downloads: {}", 
                            challenge.author,
                            challenge.rating,
                            challenge.downloads
                        );
        ...
        [_, "marketplace", "download", id] => {
            match ChallengeMarketplace::download(id) {
...
"#;
    ...
    println!("{}", "[VSCODE] Generated syntax: talon.tmLanguage.json".green());
}
