...
pub enum EventType {
    ...
    NetworkSend { data: Vec<u8> },
    NetworkReceive { data: Vec<u8> },
    RegisterModify { register: String, old_value: u64, new_value: u64 },
...
impl TimeTravelDebugger {
    ...
    pub async fn rewind_to_event(&self, event_id: u64) -> Result<(), String> {
        ...
        let event = recorder.events.iter()
    ...
    pub async fn replay_events(&self, from_event: u64, to_event: u64) -> Result<(), String> {
        let recorder = self.recorder.read().await;
        let events: Vec<ExploitEvent> = recorder.events.iter()
    ...
    async fn apply_event(&self, event: &ExploitEvent) -> Result<(), String> {
        match &event.event_type {
            EventType::MemoryWrite { address, data } => {
                println!("Replaying memory write at 0x{:x}: {} bytes", address, data.len());
            }
            EventType::RegisterModify { register, new_value, .. } => {
                self.session.set_register(register.clone(), *new_value).await;
            }
    ...
    pub async fn create_branch(&self, name: String) -> Result<BranchPoint, String> {
        let checkpoint = self.session.checkpoint_labeled(format!("branch_{}", name)).await?;
    ...
    pub async fn export_timeline(&self) -> Timeline {
        ...
        Timeline {
            events: events.clone(),
            checkpoints: checkpoints.iter().map(|c| TimelineCheckpoint {
                id: c.id,
                label: c.label.clone(),
                event_id: c.id,
            }).collect(),
    ...
    F: std::future::Future<Output = Result<T, String>>,
{
    ...
    if result.is_err() {
        ...
        println!("Exploit failed. Recorded {} events for analysis", events.len());
    }
...
mod tests {
    ...
    async fn test_branch_creation() {
        ...
        let branch = debugger.create_branch("test_branch".to_string()).await.unwrap();
