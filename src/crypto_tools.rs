...
impl WordlistGenerator {
    ...
    pub fn generate_mutations(word: &str) -> Vec<String> {
        ...
        println!(
            "[WORDLIST] Generated {} mutations for '{}'",
            mutations.len(),
            word
        );
    ...
    pub fn common_passwords() -> Vec<&'static str> {
        vec![
            ...
            "football", "admin", "root", "toor", "pass", "test",
        ]
    }

    pub fn save_wordlist(words: &[String], output_path: &str) -> Result<(), String> {
        let content = words.join("\n");
        fs::write(output_path, content).map_err(|e| format!("Failed to write wordlist: {}", e))?;
...
impl RainbowTable {
    ...
    pub fn generate(&mut self, wordlist: &[String], hash_type: &str) {
        println!(
            "[RAINBOW] Generating rainbow table for {} entries",
            wordlist.len()
        );
        ...
        println!(
            "[RAINBOW] Generated {} hash->plaintext mappings",
            self.table.len()
        );
    }
    ...
    pub fn save(&self, output_path: &str) -> Result<(), String> {
        ...
        for (hash, plaintext) in &self.table {
            writeln!(file, "{}:{}", hash, plaintext).map_err(|e| format!("Write failed: {}", e))?;
        }
...
impl BruteForceGenerator {
    ...
    pub fn generate(&self, min_len: usize, max_len: usize, max_results: usize) -> Vec<String> {
        ...
        println!(
            "[BRUTE-FORCE] Generating combinations (min:{}, max:{}, limit:{})",
