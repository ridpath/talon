...
impl ShellcodeEncoder {
    ...
    pub fn xor_encode(&self, key: u8) -> Result<Vec<u8>, String> {
        ...
        for &byte in &self.shellcode {
            ...
            if self.bad_chars.contains(&encoded) {
                return Err(format!(
                    "Key 0x{:02x} creates bad char 0x{:02x}",
                    key, encoded
                ));
            }
        }

        let encoded: Vec<u8> = self.shellcode.iter().map(|&b| b ^ key).collect();
    ...
    pub fn alphanumeric_encode(&self) -> Result<Vec<u8>, String> {
        log::info!(
            "Alphanumeric encoding shellcode ({} bytes)",
            self.shellcode.len()
        );
    ...
    pub fn xor_decoder_stub(key: u8, shellcode_len: usize) -> Vec<u8> {
        ...
        log::info!(
            "Generating XOR decoder stub (key=0x{:02x}, len={})",
            key,
            shellcode_len
        );
