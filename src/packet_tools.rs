...
impl TCPPacket {
    ...
    pub fn syn() -> u8 { TCP_FLAG_SYN }
    pub fn ack() -> u8 { TCP_FLAG_ACK }
    pub fn rst() -> u8 { TCP_FLAG_RST }
    pub fn fin() -> u8 { TCP_FLAG_FIN }
    pub fn psh() -> u8 { TCP_FLAG_PSH }
...
impl UDPPacket {
    ...
    pub fn send(&self, target_ip: &str) -> Result<(), String> {
        ...
        socket.send_to(&self.payload, &target)
...
impl ARPPacket {
    ...
    pub fn reply(sender_mac: [u8; 6], sender_ip: [u8; 4], target_mac: [u8; 6], target_ip: [u8; 4]) -> Self {
        ARPPacket {
            operation: 2,
            sender_mac,
            sender_ip,
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        println!("[ARP-PACKET]   Operation: {}", 
            if self.operation == 1 { "REQUEST" } else { "REPLY" });
        println!("[ARP-PACKET]   Sender IP: {}.{}.{}.{}", 
            self.sender_ip[0], self.sender_ip[1], self.sender_ip[2], self.sender_ip[3]);
        println!("[ARP-PACKET]   Target IP: {}.{}.{}.{}", 
...
impl SYNScanner {
    pub fn scan_port(target: &str, port: u16, timeout_ms: u64) -> Result<bool, String> {
        ...
        let result = TcpStream::connect_timeout(
            &target_addr.parse().map_err(|e| format!("Invalid address: {}", e))?,
    ...
    pub fn scan_range(target: &str, start_port: u16, end_port: u16) -> Result<Vec<u16>, String> {
        ...
        for port in start_port..=end_port {
            ...
            if (port - start_port) % 100 == 0 {
                println!("[SYN-SCAN] Progress: {} ports scanned...", port - start_port + 1);
            }
        }
        
        println!("[SYN-SCAN] Scan complete. Found {} open ports", open_ports.len());
...
impl HTTPRequest {
    ...
    pub fn post(path: &str, body: Vec<u8>) -> Self {
        ...
        req.body = body.clone();
        req.headers.push(("Content-Length".to_string(), body.len().to_string()));
        req
    }
    ...
    pub fn send(&self, host: &str, port: u16) -> Result<String, String> {
        ...
        println!("[HTTP-PACKET] [OK] Response received ({} bytes)", response.len());
...
impl PacketSniffer {
    pub fn sniff_interface(interface: &str, count: u32) -> Result<(), String> {
        println!("[SNIFFER] Starting packet capture on {} ({} packets)", interface, count);
...
impl FloodTools {
    pub fn udp_flood(target: &str, port: u16, count: u32, payload_size: usize) -> Result<(), String> {
        println!("[FLOOD] WARNING: UDP flood attack on {}:{}", target, port);
        println!("[FLOOD] Packets: {}, Payload size: {} bytes", count, payload_size);
