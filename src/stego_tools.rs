...
impl LSBExtractor {
    ...
    pub fn extract_from_image(image_path: &str) -> Result<Vec<u8>, String> {
        ...
        if let Ok(text) = String::from_utf8(extracted.clone()) {
            if text.chars().take(100).all(|c| c.is_ascii() && !c.is_control() || c == '\n') {
    ...
    pub fn hide_in_image(image_path: &str, data: &[u8], output_path: &str) -> Result<(), String> {
        let mut image_data = fs::read(image_path)
            .map_err(|e| format!("Failed to read image: {}", e))?;
        ...
        if image_data.len() < data_bits.len() + BMP_HEADER_SIZE {
            return Err(format!("Image too small for data: need {} bytes, have {} bytes", 
                data_bits.len() + BMP_HEADER_SIZE, image_data.len()));
        }
        ...
        for (offset, window) in data.windows(4).enumerate() {
            for (sig, name) in &signatures {
                if window.starts_with(sig) {
                    ...
                    println!("[FILE-SIG] Found {} signature at offset 0x{:x}", name, offset);
                }
            }
        }
...
impl StringExtractor {
    pub fn extract_strings(data: &[u8], min_length: usize) -> Vec<String> {
        ...
        println!("[STRINGS] Extracted {} strings (min length: {})", strings.len(), min_length);
        strings
    }
    ...
    pub fn extract_base64_strings(data: &[u8]) -> Vec<String> {
        ...
        println!("[STRINGS] Found {} potential base64 strings", base64_strings.len());
        base64_strings
    }
...
impl AudioStego {
    pub fn analyze_wav(file_path: &str) -> Result<(), String> {
        ...
        while pos + 8 <= data.len() {
            ...
            println!("[AUDIO-STEGO] Found chunk: {} (size: {})", chunk_name, chunk_size);
    ...
    pub fn extract_lsb_from_audio(file_path: &str) -> Result<Vec<u8>, String> {
        ...
        println!("[AUDIO-STEGO] Extracted {} bytes from audio LSB", extracted.len());
        Ok(extracted)
    }
}
...
impl MetadataExtractor {
    ...
    fn basic_metadata(file_path: &str) -> Result<(), String> {
        ...
        println!("[METADATA] Read-only: {}", metadata.permissions().readonly());
    ...
    pub fn extract_from_pdf(file_path: &str) -> Result<Vec<String>, String> {
        ...
        for line in content.lines() {
            if line.contains("/Author") || line.contains("/Creator") ||
               line.contains("/Producer") || line.contains("/Title") {
...
impl SpectralAnalyzer {
    pub fn analyze_for_hidden_messages(file_path: &str) -> Result<(), String> {
        println!("[SPECTRAL] Analyzing {} for hidden spectrogram messages", file_path);
