...
impl BinaryPatcher {
    pub fn patch_bytes(file_path: &str, offset: usize, new_bytes: &[u8], output_path: &str) -> Result<(), String> {
        println!("[BINARY-PATCH] Patching {} at offset 0x{:x}", file_path, offset);
        
        let mut data = fs::read(file_path)
            .map_err(|e| format!("Failed to read binary: {}", e))?;
        
        if offset + new_bytes.len() > data.len() {
            return Err(format!("Patch extends beyond file (file size: {}, patch end: {})", 
                data.len(), offset + new_bytes.len()));
        }
        
        println!("[BINARY-PATCH] Original bytes at 0x{:x}: {:02x?}", 
            offset, &data[offset..offset+new_bytes.len()]);
        
        data[offset..offset+new_bytes.len()].copy_from_slice(new_bytes);
        
        println!("[BINARY-PATCH] New bytes at 0x{:x}: {:02x?}", offset, new_bytes);
        
        fs::write(output_path, data)
            .map_err(|e| format!("Failed to write patched binary: {}", e))?;
        
        println!("[BINARY-PATCH] [OK] Patched binary saved to {}", output_path);
        
        Ok(())
    }
    
    pub fn nop_instructions(file_path: &str, start_offset: usize, count: usize, output_path: &str) -> Result<(), String> {
        println!("[BINARY-PATCH] NOP-ing {} bytes starting at 0x{:x}", count, start_offset);
        
        let nops = vec![0x90; count];
        Self::patch_bytes(file_path, start_offset, &nops, output_path)
    }
    
    pub fn replace_call(file_path: &str, call_offset: usize, new_target: u32, output_path: &str) -> Result<(), String> {
        println!("[BINARY-PATCH] Replacing CALL at 0x{:x} with target 0x{:x}", 
        ...
        if data[call_offset] != 0xE8 {
            return Err(format!("Expected CALL instruction (0xE8) at 0x{:x}, found 0x{:02x}", 
    ...
    pub fn replace_jump(file_path: &str, jump_offset: usize, new_target: u32, output_path: &str) -> Result<(), String> {
        println!("[BINARY-PATCH] Replacing JMP at 0x{:x} with target 0x{:x}", 
        ...
        if data[jump_offset] != 0xE9 && data[jump_offset] != 0xEB {
            return Err(format!("Expected JMP instruction at 0x{:x}, found 0x{:02x}", 
    ...
    pub fn patch_string(file_path: &str, old_string: &str, new_string: &str, output_path: &str) -> Result<usize, String> {
        println!("[BINARY-PATCH] Replacing '{}' with '{}'", old_string, new_string);
...
impl HexEditor {
    pub fn display(file_path: &str, offset: usize, length: usize) -> Result<(), String> {
        ...
        println!("[HEX-EDITOR] Displaying {} bytes from offset 0x{:x}:", read, offset);
        println!();
    ...
    pub fn search_hex(file_path: &str, hex_pattern: &str) -> Result<Vec<usize>, String> {
        ...
        println!("[HEX-EDITOR] Found {} matches for pattern {}", offsets.len(), hex_pattern);
    ...
    pub fn compare_files(file1: &str, file2: &str) -> Result<Vec<usize>, String> {
        ...
        for offset in differences.iter().take(10) {
            println!("[HEX-EDITOR]   0x{:x}: 0x{:02x} vs 0x{:02x}", 
...
impl ShellcodeInjector {
    pub fn inject_at_entry(binary_path: &str, shellcode: &[u8], output_path: &str) -> Result<(), String> {
        println!("[SHELLCODE-INJ] Injecting {} bytes at entry point", shellcode.len());
    ...
    pub fn create_code_cave(binary_path: &str, size: usize, output_path: &str) -> Result<usize, String> {
        ...
        println!("[SHELLCODE-INJ] [OK] Code cave created at offset 0x{:x}", cave_offset);
...
impl ChecksumFixer {
    pub fn recalculate_pe_checksum(pe_path: &str, output_path: &str) -> Result<(), String> {
        ...
        let mut data = fs::read(pe_path)
            .map_err(|e| format!("Failed to read PE: {}", e))?;
        ...
        fs::write(output_path, data)
            .map_err(|e| format!("Failed to write PE: {}", e))?;
        
        println!("[CHECKSUM-FIX] [OK] Checksum recalculated: 0x{:08x}", checksum);
...
impl SignatureBreaker {
    pub fn flip_random_bits(file_path: &str, num_flips: usize, output_path: &str) -> Result<(), String> {
        ...
        for _ in 0..num_flips {
            ...
            println!("[SIG-BREAKER] Flipped bit {} at offset 0x{:x}", bit_offset, byte_offset);
        }
        
        fs::write(output_path, data)
            .map_err(|e| format!("Failed to write file: {}", e))?;
        
        println!("[SIG-BREAKER] [OK] Modified binary saved to {}", output_path);
        
        Ok(())
    }
    
    pub fn append_garbage(file_path: &str, garbage_size: usize, output_path: &str) -> Result<(), String> {
        ...
        println!("[SIG-BREAKER] [OK] Garbage appended, saved to {}", output_path);
