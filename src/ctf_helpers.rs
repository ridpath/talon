...
impl FlagFinder {
    ...
    pub fn find_in_binary(file_path: &str) -> Result<Vec<String>, String> {
        let data = fs::read(file_path).map_err(|e| format!("Failed to read binary: {}", e))?;
    ...
    pub fn custom_pattern(text: &str, pattern: &str) -> Result<Vec<String>, String> {
        ...
        println!(
            "[FLAG-FINDER] Found {} matches for custom pattern",
            matches.len()
        );
        Ok(matches)
    }
...
impl CTFPatterns {
    pub fn check_for_patterns(text: &str) -> HashMap<String, Vec<String>> {
        ...
        let base64_matches: Vec<String> = base64_regex
            .find_iter(text)
        ...
        let hex_matches: Vec<String> = hex_regex
            .find_iter(text)
            .map(|m| m.as_str().to_string())
            .collect();
        ...
        let url_matches: Vec<String> = url_regex
            .find_iter(text)
            .map(|m| m.as_str().to_string())
            .collect();
        ...
        let ip_matches: Vec<String> = ip_regex
            .find_iter(text)
            .map(|m| m.as_str().to_string())
            .collect();
        ...
        let hash32_matches: Vec<String> = hash32_regex
            .find_iter(text)
            .map(|m| m.as_str().to_string())
            .collect();
        ...
        let hash40_matches: Vec<String> = hash40_regex
            .find_iter(text)
            .map(|m| m.as_str().to_string())
            .collect();
...
"#
    ...
    pub fn save_template(template_type: &str, output_path: &str) -> Result<(), String> {
        ...
        fs::write(output_path, content).map_err(|e| format!("Failed to write template: {}", e))?;
...
"#
            ...
            _ => {
                println!(
                    "[CTF-CHEATSHEET] Unknown category. Available: pwn, web, crypto, forensics"
                );
            }
        }
    }
