...
impl TaintTracker {
    ...
    pub fn track_propagation(&mut self, src_addr: u64, dst_addr: u64, operation: &str) {
        if let Some(taint) = self.taint_map.get(&src_addr).cloned() {
            ...
            new_taint.propagation_chain.push(format!(
                "{}: 0x{:x} → 0x{:x}",
                operation, src_addr, dst_addr
            ));
    ...
    pub fn check_leak(&mut self, output: &[u8], sink: TaintSink) -> Vec<TaintLeak> {
        ...
        for (i, window) in output.windows(8).enumerate() {
            ...
            if self.is_pie_base(value) {
                ...
                println!(
                    "[TAINT] WARNING: PIE BASE LEAK: 0x{:016x} at offset {}",
                    value, i
                );
            }
            ...
            if self.is_libc_address(value) {
                ...
                println!(
                    "[TAINT] WARNING: LIBC ADDRESS LEAK: 0x{:016x} at offset {}",
                    value, i
                );
            }
        }
    ...
    fn is_canary(&self, value: u64) -> bool {
        (value & 0xff) == 0x00 && (value >> 8) != 0 && (value & 0xff00000000000000) != 0
    }
    ...
    pub fn analyze_binary(&mut self, binary_path: &str) -> Result<TaintAnalysisResult, String> {
        ...
        println!(
            "[TAINT] [OK] Analysis complete: {} leaks detected",
            total_leaks
        );
        ...
        Ok(TaintAnalysisResult {
            ...
            high_count: self
                .detected_leaks
                .iter()
                .filter(|l| l.severity == LeakSeverity::High)
                .count(),
        })
    }
    ...
    fn execute_with_taint_tracking(
        &self,
        binary_path: &str,
        input: &[u8],
    ...
    fn save_leak_report(
        ...
        let report_path = format!(
            "taint_leak_{}_{}.txt",
            Path::new(binary).file_name().unwrap().to_str().unwrap(),
            test_id
        );
...
impl ProtocolFuzzer {
    ...
    pub fn fuzz(&mut self) -> Result<FuzzResult, String> {
        log::info!(
            "Starting GOD-MODE fuzzing campaign ({} iterations)",
            self.max_iterations
        );
        ...
        while iterations < self.max_iterations {
            ...
            if let Some(crash) = self.execute_test_case(&test_case)? {
                if self.crash_hashes.insert(crash.crash_hash.clone()) {
                    log::warn!(
                        "Unique crash found! Exploitability: {:.1}/100",
                        crash.exploitability_score
                    );
            ...
            if iterations % 10000 == 0 {
                log::info!(
                    "Progress: {} iters | {} corpus | {} edges | {} crashes",
                    iterations,
                    self.corpus.len(),
                    self.coverage_map.total_edges(),
                    unique_crashes
    ...
    fn execute_test_case(&self, test_case: &[u8]) -> Result<Option<Crash>, String> {
        ...
        if is_crash {
            ...
            return Ok(Some(Crash {
                ...
                backtrace: vec![
                    "vulnerable_func+0x42".to_string(),
                    "main+0x1234".to_string(),
                ],
                severity,
                crash_hash,
    ...
    fn compute_exploitability(&self, input: &[u8]) -> f64 {
        ...
        if input.windows(4).any(|w| w == b"AAAA") {
            score += 10.0;
        }
        if input.windows(3).any(|w| w == b"%s%") {
            score += 15.0;
        }
