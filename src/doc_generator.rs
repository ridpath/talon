...
impl DocGenerator {
    ...
    fn populate_stdlib_docs(&mut self) {
        ...
        self.add_function(FunctionDoc {
            ...
            description: "Generate a De Bruijn sequence for finding buffer overflow offsets".to_string(),
            parameters: vec![
                ParameterDoc {
        ...
        self.add_function(FunctionDoc {
            ...
            description: "Unpack 8 bytes to 64-bit little-endian integer. Used to parse leaked addresses.".to_string(),
            parameters: vec![
                ParameterDoc {
                    name: "data".to_string(),
                    type_hint: "bytes".to_string(),
            ...
            module: "packing".to_string(),
            tags: vec!["packing".to_string(), "bytes".to_string(), "leak".to_string()],
        });
        ...
        self.add_function(FunctionDoc {
            ...
            examples: vec![
                "let leaked_addr = u32(response[0:4])".to_string(),
            ],
            module: "packing".to_string(),
            tags: vec!["packing".to_string(), "bytes".to_string(), "x86".to_string()],
        });
        ...
        self.add_function(FunctionDoc {
            ...
            description: "Send data with newline appended. Returns number of bytes sent including newline.".to_string(),
            parameters: vec![
                ParameterDoc {
                    name: "conn".to_string(),
                    type_hint: "connection".to_string(),
        ...
        self.add_function(FunctionDoc {
            ...
            description: "Receive data until newline. Returns data including the newline.".to_string(),
            parameters: vec![
                ParameterDoc {
                    name: "conn".to_string(),
                    type_hint: "connection".to_string(),
    ...
    pub fn display_function(&self, name: &str) {
        if let Some(doc) = self.get_function_doc(name) {
            ...
            println!("{} {}", "[ERROR]".red(), format!("Function '{}' not found in documentation.", name).red());
            ...
            if !similar.is_empty() {
                ...
                println!("\n{} Use help() to see all available functions", "TIP:".yellow());
            }
        }
    }
    ...
    pub fn display_search_results(&self, query: &str) {
        ...
        if results.is_empty() {
            println!("{} {}", "[SEARCH]".yellow(), format!("No functions found matching '{}'", query).bright_black());
            return;
        }
        
        println!("\n{} {} {}", "[SEARCH]".yellow(), "Search Results for".cyan().bold(), query.green().bold());
        ...
        for doc in results {
            ...
            println!("   {} {} | {} {}", 
                "Module:".bright_black(), doc.module.yellow(),
                "Tags:".bright_black(), doc.tags.join(", ").magenta()
            );
            println!();
        }
    }
    ...
    fn print_function_doc(&self, doc: &FunctionDoc) {
        println!("\n{}", "═".repeat(80).cyan());
        println!("  {} {}", "[DOC]".blue(), doc.name.to_uppercase().cyan().bold());
        println!("{}\n", "═".repeat(80).cyan());
        ...
        if !doc.examples.is_empty() {
            ...
            for (idx, example) in doc.examples.iter().enumerate() {
                println!("  {} {}", format!("{}.", idx + 1).bright_black(), example.cyan());
            }
            println!();
        }
        ...
        println!("{} {}", "MODULE:".bright_black(), doc.module.yellow().bold());
        println!();
    }
            ...
            if let Some(functions) = self.modules.get(module.as_str()) {
                for func_name in functions {
                    if let Some(doc) = self.functions.get(func_name) {
                        println!("  {:20} - {}", doc.name, doc.description.chars().take(50).collect::<String>());
                    }
                }
            }
            println!();
