...
impl SimilarityEngine {
    ...
    fn init_vulnerable_patterns(&mut self) {
        ...
        self.known_vulnerable_patterns.insert(
            ...
            vec![0.92, 0.15, 0.78, 0.34, 0.89, 0.12, 0.67, 0.45, 0.91, 0.23, 0.56, 0.78, 0.34, 0.89, 0.45, 0.67],
        );
        
        self.known_vulnerable_patterns.insert(
            "gets_dangerous".to_string(),
            vec![0.88, 0.23, 0.71, 0.45, 0.82, 0.19, 0.63, 0.51, 0.87, 0.29, 0.54, 0.76, 0.41, 0.85, 0.48, 0.69],
        );
        
        self.known_vulnerable_patterns.insert(
            "sprintf_overflow".to_string(),
            vec![0.85, 0.31, 0.69, 0.52, 0.79, 0.25, 0.61, 0.58, 0.84, 0.35, 0.52, 0.74, 0.47, 0.81, 0.53, 0.65],
        );
        
        self.known_vulnerable_patterns.insert(
            "system_injection".to_string(),
            vec![0.91, 0.18, 0.76, 0.38, 0.86, 0.14, 0.65, 0.48, 0.89, 0.26, 0.58, 0.77, 0.36, 0.87, 0.46, 0.68],
        );
        
        self.known_vulnerable_patterns.insert(
            "uaf_pattern".to_string(),
            vec![0.94, 0.12, 0.81, 0.29, 0.92, 0.09, 0.71, 0.41, 0.93, 0.19, 0.62, 0.82, 0.31, 0.91, 0.43, 0.72],
        );
        
        println!("[SIMILARITY] [OK] Loaded {} vulnerable patterns", self.known_vulnerable_patterns.len());
    }
    
    fn init_vendor_signatures(&mut self) {
        println!("[SIMILARITY] 🏢 Loading vendor code signatures...");
        
        self.vendor_signatures.insert("glibc_2.31".to_string(), vec![
            vec![0.87, 0.34, 0.65, 0.45, 0.78, 0.29, 0.56, 0.61, 0.82, 0.38, 0.59, 0.71, 0.42, 0.84, 0.51, 0.66],
            vec![0.89, 0.31, 0.68, 0.41, 0.81, 0.26, 0.59, 0.64, 0.85, 0.35, 0.62, 0.74, 0.39, 0.87, 0.48, 0.69],
        ]);
        
        self.vendor_signatures.insert("openssl_1.1.1".to_string(), vec![
            vec![0.91, 0.28, 0.72, 0.38, 0.84, 0.23, 0.63, 0.67, 0.88, 0.32, 0.65, 0.77, 0.36, 0.90, 0.45, 0.72],
            vec![0.93, 0.25, 0.75, 0.34, 0.87, 0.19, 0.66, 0.71, 0.91, 0.29, 0.68, 0.80, 0.33, 0.93, 0.42, 0.75],
        ]);
        
        self.vendor_signatures.insert("zlib_1.2.11".to_string(), vec![
            vec![0.82, 0.39, 0.61, 0.49, 0.74, 0.34, 0.52, 0.57, 0.78, 0.43, 0.55, 0.67, 0.46, 0.80, 0.54, 0.62],
        ]);
        
        println!("[SIMILARITY] [OK] Loaded {} vendor signatures", self.vendor_signatures.len());
    }
    ...
    pub fn analyze_similarity(
        ...
        println!("[SIMILARITY] Similarity Threshold: {:.2}%", threshold * 100.0);
        println!("[SIMILARITY] ═══════════════════════════════════════════════════════════════\n");
        ...
        println!("[SIMILARITY] [OK] Extracted {} functions from reference binary", reference_functions.len());
        ...
        for pattern in search_patterns {
            ...
            println!("[SIMILARITY]   Found {} binaries matching pattern", paths.len());
            ...
            for path in paths {
                ...
                if self.is_binary_file(&path_str) {
                    ...
                    match self.extract_functions(&path_str) {
                        Ok(functions) => {
                            println!("[SIMILARITY]      [OK] Extracted {} functions", functions.len());
        ...
        for ref_func in &reference_functions {
            for candidate_func in &all_candidate_functions {
                let similarity = self.compute_similarity(&ref_func.features, &candidate_func.features);
                
                if similarity >= threshold {
                    let confidence = self.calculate_confidence(similarity, ref_func, candidate_func);
                    let match_type = self.determine_match_type(similarity, ref_func, candidate_func);
                    let evidence = self.collect_evidence(ref_func, candidate_func, similarity);
                    let vulnerable_indicators = self.check_vulnerable_patterns(&candidate_func.features);
    ...
    fn extract_functions_unix(&self, binary_path: &str, arch: &str) -> Result<Vec<FunctionEmbedding>, String> {
    ...
    fn extract_functions_windows(&self, binary_path: &str, arch: &str) -> Result<Vec<FunctionEmbedding>, String> {
    ...
    fn parse_nm_output(&self, output: &[u8], binary: &str, arch: &str) -> Result<Vec<FunctionEmbedding>, String> {
        let mut functions = Vec::new();
    ...
    fn parse_readelf_output(&self, output: &[u8], binary: &str, arch: &str) -> Result<Vec<FunctionEmbedding>, String> {
        let mut functions = Vec::new();
        let output_str = String::from_utf8_lossy(output);
    ...
    fn parse_dumpbin_output(&self, output: &[u8], binary: &str, arch: &str) -> Result<Vec<FunctionEmbedding>, String> {
        let mut functions = Vec::new();
        let output_str = String::from_utf8_lossy(output);
    ...
    fn generate_function_embedding(&self, name: &str, address: u64, binary: &str, arch: &str) -> Vec<f32> {
        ...
        features[2] = if name.contains("strcpy") || name.contains("sprintf") || name.contains("gets") {
            0.85
        } else {
            0.15
        };
        
        features[3] = if name.contains("malloc") || name.contains("free") || name.contains("realloc") {
            0.78
        } else {
            0.22
        };
        ...
        let binary_hash = binary.bytes().fold(0u64, |acc, b| acc.wrapping_add(b as u64));
    ...
    fn determine_match_type(&self, similarity: f64, func1: &FunctionEmbedding, func2: &FunctionEmbedding) -> MatchType {
    ...
    fn collect_evidence(&self, func1: &FunctionEmbedding, func2: &FunctionEmbedding, similarity: f64) -> Vec<String> {
        let mut evidence = Vec::new();
        
        evidence.push(format!("Cosine similarity: {:.3}", similarity));
        evidence.push(format!("Architecture match: {} vs {}", func1.architecture, func2.architecture));
        
        if func1.name == func2.name {
            evidence.push(format!("Identical function names: {}", func1.name));
        }
        
        evidence.push(format!("Size comparison: {} vs {} bytes", func1.size, func2.size));
        evidence.push(format!("Instruction count: {} vs {}", func1.instruction_count, func2.instruction_count));
        evidence.push(format!("Cyclomatic complexity: {} vs {}", func1.cyclomatic_complexity, func2.cyclomatic_complexity));
    ...
    fn check_vulnerable_patterns(&self, features: &[f32]) -> Vec<String> {
        ...
        for (pattern_name, pattern_features) in &self.known_vulnerable_patterns {
            ...
            if similarity >= 0.75 {
                indicators.push(format!("{} pattern detected ({:.1}% match)", pattern_name, similarity * 100.0));
            }
        }
    ...
    fn is_binary_file(&self, path: &str) -> bool {
        if cfg!(target_os = "windows") {
            ...
            if let Ok(out) = output {
                ...
                result.contains("ELF") || result.contains("executable") || result.contains("shared object")
            } else {
                false
            }
        }
