...
impl ZipTools {
    ...
    pub fn list_contents(zip_path: &str) -> Result<Vec<String>, String> {
        ...
        for i in 0..archive.len() {
            ...
            println!("[ZIP]   {} ({} bytes, compressed: {} bytes)", 
    ...
    pub fn crack_password(zip_path: &str, wordlist: &str) -> Result<Option<String>, String> {
        ...
        let passwords = fs::read_to_string(wordlist)
...
impl TarTools {
        ...
        match output {
            Ok(out) => {
                if out.status.success() {
                    ...
                    Err(format!("tar failed: {}", String::from_utf8_lossy(&out.stderr)))
                }
            }
            Err(_) => {
                Err("tar command not found".to_string())
            }
        }
    }
    ...
    pub fn list_contents(tar_path: &str) -> Result<Vec<String>, String> {
        ...
        if output.status.success() {
            ...
            let files: Vec<String> = contents.lines()
                .map(|s| s.to_string())
                .collect();
...
impl RarTools {
        ...
        match output {
            Ok(out) => {
                if out.status.success() {
                    ...
                    Err(format!("unrar failed: {}", String::from_utf8_lossy(&out.stderr)))
                }
            }
            Err(_) => {
    ...
    pub fn list_contents(rar_path: &str) -> Result<Vec<String>, String> {
        ...
        if output.status.success() {
            ...
            Err(format!("unrar failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }
}
...
impl SevenZipTools {
        ...
        match output {
            Ok(out) => {
                if out.status.success() {
                    ...
                    Err(format!("7z failed: {}", String::from_utf8_lossy(&out.stderr)))
                }
            }
            Err(_) => {
    ...
    pub fn create(files: &[&str], output_archive: &str) -> Result<(), String> {
        ...
        if output.status.success() {
            ...
            Err(format!("7z failed: {}", String::from_utf8_lossy(&output.stderr)))
        }
    }
