...
pub fn set_timestamps(_path: &str, _modified: SystemTime, _accessed: SystemTime) -> Result<(), String> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::MetadataExt;
        use std::ffi::CString;
        ...
        let accessed_secs = accessed.duration_since(UNIX_EPOCH)
            .map_err(|e| format!("Invalid time: {}", e))?
            .as_secs() as i64;
        ...
        unsafe {
            if libc::utime(path_c.as_ptr(), &times as *const Utimbuf as *const libc::utimbuf) == 0 {
...
pub fn read_alternate_data_stream(path: &str, stream_name: &str) -> Result<String, String> {
    #[cfg(target_os = "windows")]
    {
        ...
        fs::read_to_string(&ads_path)
            .map_err(|e| format!("Failed to read ADS: {}", e))
    }
...
pub fn write_alternate_data_stream(path: &str, stream_name: &str, data: &str) -> Result<(), String> {
    #[cfg(target_os = "windows")]
    {
        let ads_path = format!("{}:{}", path, stream_name);
        fs::write(&ads_path, data)
            .map_err(|e| format!("Failed to write ADS: {}", e))?;
...
pub fn list_open_files() -> Result<Vec<String>, String> {
    #[cfg(target_os = "linux")]
    {
        ...
        let entries = fs::read_dir(&fd_path)
            .map_err(|e| format!("Failed to read fd directory: {}", e))?;
...
pub fn clear_file_slack_space(path: &str) -> Result<(), String> {
    ...
    if slack_size > 0 && slack_size < block_size {
        ...
        println!("[FS] Cleared {} bytes of slack space in {}", slack_size, path);
    }
