...
impl CyclicPattern {
    ...
    pub fn generate(&self, length: usize) -> Vec<u8> {
        ...
        fn db(
            ...
            if t > n {
                ...
                if t < a.len() && (t - p) < a.len() {
                    a[t] = a[t - p];
                    db(t + 1, p, sequence, a, alphabet_len, n, result, alphabet, max_len);
                    ...
                    for j in (a[t - p] + 1)..alphabet_len {
                        ...
                        if t < a.len() && t < sequence.len() {
                            a[t] = j;
                            sequence[t] = j;
                            db(t + 1, t, sequence, a, alphabet_len, n, result, alphabet, max_len);
                        }
                    }
                }
            }
        ...
        db(1, 1, &mut sequence, &mut a, alphabet_len, self.n, &mut pattern, &self.alphabet, length);
...
pub fn cyclic(length: usize) -> Vec<u8> {
    ...
    if length == 0 {
        eprintln!("{} {}", "[WARNING]".yellow(), "Requested cyclic pattern length is 0, returning empty pattern".bright_black());
        return Vec::new();
    }
    if length > 1_000_000 {
        eprintln!("{} {}", "[WARNING]".yellow(), format!("Requested pattern length {} exceeds 1MB, this may take a while", length).yellow());
    }
...
pub fn cyclic_find(pattern_bytes: &[u8], search_value: &str) -> Option<usize> {
    ...
    if pattern_bytes.is_empty() {
        eprintln!("{} {}", "[ERROR]".red(), "Cannot search in empty pattern".red());
        return None;
    }
    
    if search_value.is_empty() {
        eprintln!("{} {}", "[ERROR]".red(), "Search value cannot be empty".red());
        return None;
    }
    ...
    if search_value.starts_with("0x") {
        if let Ok(value) = u64::from_str_radix(&search_value[2..], 16) {
            ...
            if result.is_none() {
                eprintln!("{} {}", "[INFO]".yellow(), format!("Value {} not found in pattern. Pattern may be too short.", search_value).cyan());
            }
            return result;
        }
    }
    ...
    if result.is_none() {
        eprintln!("{} {}", "[INFO]".yellow(), format!("String '{}' not found in pattern. Check for typos or generate a longer pattern.", search_value).cyan());
    }
    result
}
...
mod tests {
    ...
    fn test_cyclic_generation() {
        ...
        assert!(pattern.windows(4).enumerate().all(|(i, w)| {
            pattern.windows(4).skip(i + 1).all(|other| w != other)
        }));
    }
