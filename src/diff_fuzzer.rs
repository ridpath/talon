...
impl DifferentialFuzzer {
    ...
    pub fn fuzz(&mut self) -> Result<(), String> {
        ...
        for i in 0..self.iterations {
            ...
            if (i + 1) % 1000 == 0 {
                println!(
                    "[DIFF-FUZZ] Progress: {}/{} iterations, {} divergences found",
                    i + 1,
                    self.iterations,
                    divergences_found
                );
    ...
    fn execute_target(&self, target: &str, input: &[u8]) -> Result<ExecutionResult, String> {
        ...
        let asan_violation = String::from_utf8_lossy(&output.stderr).contains("AddressSanitizer")
            ...
            match mode {
                ...
                DetectionMode::TimingDivergence => {
                    let time_diff = (old_result.execution_time_ms as i64
                        - new_result.execution_time_ms as i64)
                        .abs();
    ...
    fn save_divergence(&self, divergence: &DivergenceCase, id: usize) -> Result<(), String> {
        ...
        fs::write(&report_filename, report).map_err(|e| format!("Failed to save report: {}", e))?;
...
"#,
        ...
        fs::write(&poc_filename, poc_code).map_err(|e| format!("Failed to generate PoC: {}", e))?;
