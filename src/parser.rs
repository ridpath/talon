use crate::ast::{
    ASLRBypassSpec, AutoExploitSpec, AutoPatchSpec, BinaryDiffSpec, BinarySimilaritySpec,
    CVEScanSpec, CloudExploitSpec, Command, ContainerEscapeSpec, Control, DebugSpec, DecompileSpec,
    DecompileTarget, DistributeSpec, Expr, FunctionDef, FuzzProtocolSpec, HeapGroomSpec,
    KernelExploitSpec, Literal, MacroDef, MatchArm, MatchBlock, OffensiveCommand,
        ...
        if pair.as_rule() == Rule::program {
            for inner_pair in pair.into_inner() {
                match inner_pair.as_rule() {
                    ...
                    Rule::command => {
                        commands.extend(parse_command(inner_pair.into_inner().next().unwrap()))
                    }
                    Rule::EOI => {}
...
fn parse_stmt(pair: Pair<Rule>) -> Result<Vec<Command>, String> {
    ...
    match pair.as_rule() {
        ...
        Rule::import_stmt => {
            ...
            let items = inner
                .next()
                .map(|items| items.into_inner().map(|i| i.as_str().to_string()).collect());
        ...
        Rule::function_def => {
            ...
            let (is_async, name) = if first.as_str() == "async" {
                let name_pair = inner.next().ok_or_else(|| {
                    format!(
                        "Missing function name after async at line {}:{}",
                        span.start_pos().line_col().0,
                        span.start_pos().line_col().1
                    )
                })?;
        ...
        Rule::call_func => Ok(vec![Command::Expr(parse_expr(pair))]),
        Rule::var_decl => {
            let mut parts = pair.into_inner();
            ...
            let (type_hint, expr_part) = if next_part.as_rule() == Rule::type_hint {
                ...
                (
                    hint,
                    parts.next().ok_or("Missing var value after type hint")?,
                )
            } else {
                (TypeHint::Unknown, next_part)
            };
            let expr = parse_expr(expr_part);
            Ok(vec![Command::TypedDecl(TypedVar {
                name,
                var_type: type_hint,
                value: expr,
            })])
...
fn parse_command(pair: Pair<Rule>) -> Vec<Command> {
    match pair.as_rule() {
        ...
        Rule::stack_overflow_cmd => {
            ...
            let addr =
                u64::from_str_radix(parts.next().unwrap().as_str().trim_start_matches("0x"), 16)
                    .unwrap();
            vec![Command::StackOverflowExploit {
                padding,
                ret_addr: addr,
            }]
        }
        ...
        Rule::symbolic_cmd => {
            ...
            vec![Command::SymbolicExecution(SymbolicSpec {
                var_name,
                var_type,
                size,
            })]
        ...
        Rule::chain_cmd => {
            ...
            if text.starts_with("connect_to") {
                ...
                let offset = u64::from_str_radix(
                    parts.next().unwrap().as_str().trim_start_matches("0x"),
                    16,
                )
                .unwrap_or(0);
                parts.next();
                let name = parts.next().unwrap().as_str().trim_matches('"').to_string();
                vec![Command::ChainCalculateBase {
                    leaked_addr,
                    offset,
                    name,
                }]
                ...
                parts.next();
                let offset = parts.next().unwrap().as_str().parse::<usize>().unwrap_or(0);
                vec![Command::ChainBruteforceASLR {
                    attempts,
                    payload,
                    offset,
                }]
        ...
        Rule::decompile_cmd => {
            ...
            let target = if target_str.starts_with("0x") {
                let addr =
                    u64::from_str_radix(target_str.trim_start_matches("0x"), 16).unwrap_or(0);
        ...
        _ => vec![Command::Expr(Expr::Literal(Literal::String(format!(
            "[UNHANDLED COMMAND] {}",
            pair.as_str()
        ))))],
    }
...
fn parse_offensive_command(pair: Pair<Rule>) -> Vec<Command> {
    ...
    match cmd.as_rule() {
        Rule::ghidra_bridge_cmd => {
            ...
            let binary = parts.next().unwrap().as_str().trim_matches('"').to_string();
            cmds.push(Command::Offensive(OffensiveCommand::BridgeGhidra {
                script,
                binary,
            }));
        }
        Rule::ida_bridge_cmd => {
            ...
            let script = parts.next().unwrap().as_str().trim_matches('"').to_string();
            let binary = parts.next().unwrap().as_str().trim_matches('"').to_string();
            cmds.push(Command::Offensive(OffensiveCommand::BridgeIDA {
                script,
                binary,
            }));
        }
        _ => cmds.push(Command::Expr(Expr::Literal(Literal::String(
            "[UNKNOWN offensive_cmd]".into(),
        )))),
    }
    cmds
}
...
fn parse_expr(pair: Pair<Rule>) -> Expr {
    match pair.as_rule() {
        ...
        Rule::literal => {
            ...
            match inner.as_rule() {
                Rule::quoted_string => Expr::Literal(Literal::String(
                    inner.as_str().trim_matches('"').to_string(),
                )),
                Rule::multiline_string => Expr::Literal(Literal::String(
                    inner.as_str().trim_matches('"').replace("\"\"\"", ""),
                )),
                Rule::number => {
                ...
                Rule::byte_array => Expr::Literal(Literal::ByteArray(
                    inner.as_str().trim_start_matches("0x").to_string(),
                )),
                _ => unreachable!(),
            }
        }
