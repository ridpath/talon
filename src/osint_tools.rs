...
impl SubdomainEnumerator {
    ...
    pub fn common_subdomains() -> Vec<&'static str> {
        vec![
            ...
            "server", "mx1", "chat", "wap", "my", "svn",
            "mail1", "sites", "proxy", "ads", "host", "crm",
        ]
    }
...
impl EmailHarvester {
    ...
    pub fn extract_emails(text: &str) -> Result<Vec<String>, String> {
        ...
        let mut emails: Vec<String> = email_regex.find_iter(text)
...
impl EXIFExtractor {
                ...
                if let Ok(json) = serde_json::from_str::<Vec<serde_json::Value>>(&json_str) {
                    if let Some(first) = json.first() {
                        ...
                        for (key, value) in &metadata {
                            if key.contains("GPS") || key.contains("Date") || 
                               key.contains("Make") || key.contains("Model") {
    ...
    pub fn extract_gps(file_path: &str) -> Result<Option<(f64, f64)>, String> {
        ...
        let lat = metadata.get("GPSLatitude")
            .and_then(|s| s.parse::<f64>().ok());
        let lon = metadata.get("GPSLongitude")
            .and_then(|s| s.parse::<f64>().ok());
        
        if let (Some(lat), Some(lon)) = (lat, lon) {
            println!("[EXIF] GPS Coordinates: {}, {}", lat, lon);
            println!("[EXIF] 🌍 Google Maps: https://www.google.com/maps?q={},{}", lat, lon);
            Ok(Some((lat, lon)))
        } else {
    ...
    pub fn extract_info(whois_data: &str) -> HashMap<String, String> {
        ...
        for line in whois_data.lines() {
            if line.contains("Registrar:") {
                info.insert("Registrar".to_string(), line.split(':').nth(1).unwrap_or("").trim().to_string());
            }
            if line.contains("Creation Date:") || line.contains("Created:") {
                info.insert("Created".to_string(), line.split(':').nth(1).unwrap_or("").trim().to_string());
            }
            if line.contains("Expiration Date:") || line.contains("Expires:") {
                info.insert("Expires".to_string(), line.split(':').nth(1).unwrap_or("").trim().to_string());
            }
...
impl GoogleDork {
    pub fn generate_query(target: &str, dork_type: &str) -> String {
        let query = match dork_type {
            "files" => format!("site:{} filetype:pdf OR filetype:doc OR filetype:xls", target),
            "login" => format!("site:{} inurl:login OR inurl:admin OR inurl:dashboard", target),
            "exposed" => format!("site:{} intitle:index.of OR inurl:backup", target),
            "config" => format!("site:{} filetype:conf OR filetype:cfg OR filetype:ini", target),
            "sql" => format!("site:{} intext:'SQL syntax' OR intext:'mysql_fetch'", target),
        ...
        println!("[GOOGLE-DORK] URL: https://www.google.com/search?q={}", 
...
impl ShodanHelper {
    pub fn search(api_key: &str, query: &str) -> Result<String, String> {
        ...
        let client = Client::new();
        let url = format!("https://api.shodan.io/shodan/host/search?key={}&query={}", 
