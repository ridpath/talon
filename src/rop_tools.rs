...
impl RopChain {
                    ...
                    if insns.len() > 0 {
                        ...
                        for insn in insns.iter() {
                            if let Some(mnemonic) = insn.mnemonic() {
                                ...
                                instructions.push(format!("{} {}", mnemonic, operands).trim().to_string());
    ...
    fn score_gadget(instructions: &[String]) -> u32 {
        ...
        for instr in instructions {
            ...
            else if instr_lower.starts_with("int 0x80") { score += 90; }
            else if instr_lower == "ret" { score += 20; }
            else if instr_lower.starts_with("xor") && instr_lower.contains("eax") { score += 30; }
            ...
            else if instr_lower.starts_with("jmp") { score = score.saturating_sub(25); }
            else if instr_lower.starts_with("je ") || instr_lower.starts_with("jne ") { score = score.saturating_sub(20); }
        }
...
impl AutoROPSolver {
    ...
    fn try_ret2libc(&self, goal: &ROPGoal) -> Result<ROPSolution, String> {
        ...
        match goal {
            ROPGoal::System(cmd) | ROPGoal::Execve(cmd, _) => {
                let pop_rdi = self.find_gadget_pattern("pop rdi; ret")
    ...
    fn try_mprotect_rwx(&self, goal: &ROPGoal) -> Result<ROPSolution, String> {
        ...
        let pop_rdi = self.find_gadget_pattern("pop rdi; ret").ok_or("pop rdi not found")?;
        let pop_rsi = self.find_gadget_pattern("pop rsi; ret").ok_or("pop rsi not found")?;
        let pop_rdx = self.find_gadget_pattern("pop rdx; ret").ok_or("pop rdx not found")?;
    ...
    fn try_ret2syscall(&self, goal: &ROPGoal) -> Result<ROPSolution, String> {
        let pop_rax = self.find_gadget_pattern("pop rax; ret").ok_or("pop rax not found")?;
        let pop_rdi = self.find_gadget_pattern("pop rdi; ret").ok_or("pop rdi not found")?;
        let pop_rsi = self.find_gadget_pattern("pop rsi; ret").ok_or("pop rsi not found")?;
        let pop_rdx = self.find_gadget_pattern("pop rdx; ret").ok_or("pop rdx not found")?;
        let syscall = self.find_gadget_pattern("syscall").ok_or("syscall not found")?;
    ...
    fn try_srop(&self, _goal: &ROPGoal) -> Result<ROPSolution, String> {
        ...
        Ok(ROPSolution {
            ...
            payload_description: "SROP (Sigreturn-Oriented Programming) - full register control".to_string(),
            constraints_satisfied: self.check_constraints(&chain),
    ...
    fn try_jop(&self, _goal: &ROPGoal) -> Result<ROPSolution, String> {
        let jmp_gadgets: Vec<&Gadget> = self.gadget_db.iter()
    ...
    fn try_cop(&self, _goal: &ROPGoal) -> Result<ROPSolution, String> {
        let call_gadgets: Vec<&Gadget> = self.gadget_db.iter()
    ...
    fn analyze_gadgets(&mut self) {
        ...
        println!("[AUTO-ROP]   Syscall gadgets: {}", self.syscall_gadgets.len());
        println!("[AUTO-ROP]   Stack pivot gadgets: {}", self.pivot_gadgets.len());
    }
    ...
    fn find_one_gadgets(&mut self, _libc_path: &str) -> Result<(), String> {
        ...
        println!("[AUTO-ROP]   Found {} potential one-gadgets", self.one_gadgets.len());
