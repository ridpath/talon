...
fn test_typed_var_creation() {
    use talon::ast::{Expr, Literal, TypeHint, TypedVar};
...
fn test_function_def_creation() {
    use talon::ast::{Command, Expr, FunctionDef, Literal, TypeHint};
    ...
    let func = FunctionDef {
        ...
        body: vec![Command::Expr(Expr::Return(Box::new(Expr::Literal(
            Literal::Number(42),
        ))))],
        is_async: false,
    };
...
fn test_match_arm_creation() {
    use talon::ast::{Expr, Literal, MatchArm};
...
fn test_match_block_creation() {
    use talon::ast::{Expr, Literal, MatchArm, MatchBlock};
...
fn test_expr_call_with_named_args() {
    ...
    let expr = Expr::Call {
        ...
        args: vec![
            (
                Some("host".to_string()),
                Expr::Literal(Literal::String("127.0.0.1".to_string())),
            ),
            (
                Some("port".to_string()),
                Expr::Literal(Literal::Number(8080)),
            ),
        ],
    };
