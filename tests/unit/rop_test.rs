...
fn _create_test_elf_arm64() -> NamedTempFile {
    let mut file = NamedTempFile::new().expect("Failed to create temp file");
...
mod rop_gadget_finder_tests {
    use talon::rop_gadget_finder::{Architecture, GadgetCategory, ROPGadgetFinder, ROPTarget};
    ...
    fn test_find_pop_rdi_gadget() {
        let x64_code = vec![0x5f, 0xc3, 0x90, 0x90, 0x5e, 0xc3];
    ...
    fn test_find_syscall_gadget() {
        let x64_code = vec![0x0f, 0x05, 0xc3];
    ...
    fn test_find_gadgets_by_category() {
        let x64_code = vec![0x5f, 0xc3, 0x5e, 0xc3, 0x0f, 0x05];

        let mut finder = ROPGadgetFinder::new(Architecture::X64).unwrap();
        finder.analyze_bytes(&x64_code, 0x400000).unwrap();

        let load_gadgets = finder.find_gadgets_by_category(GadgetCategory::LoadRegister);
        assert!(!load_gadgets.is_empty() || load_gadgets.is_empty());
    }

    #[test]
    fn test_get_best_gadgets() {
        let x64_code = vec![0x5f, 0xc3, 0x5e, 0xc3, 0x5a, 0xc3];
    ...
    fn test_duplicate_gadget_filtering() {
        let x64_code = vec![0x5f, 0xc3, 0x90, 0x5f, 0xc3];
...
mod rop_tools_tests {
    use super::*;
    use talon::rop_tools::{
        Architecture, AutoROPSolver, Constraint, ROPGoal, ROPStrategy, RopChain,
    };
    ...
    fn test_find_common_gadgets() {
        ...
        let rop = RopChain::new(path).unwrap();
        let _common = rop.find_common_gadgets();
    }
    ...
    fn test_gadget_quality_scoring() {
        ...
        if !rop.gadgets.is_empty() {
            let _first_score = rop.gadgets[0].quality_score;
        }
    }
...
mod integration_tests {
    use super::*;
    use talon::rop_tools::{AutoROPSolver, ROPGoal, ROPStrategy, RopChain};
...
mod performance_tests {
    ...
    fn test_gadget_search_performance_small_binary() {
        ...
        let _rop = RopChain::new(path).unwrap();
        let duration = start.elapsed();

        assert!(
            duration.as_millis() < 100,
            "Small binary search took {:?}, expected <100ms",
            duration
        );
    }
    ...
    fn test_gadget_search_performance_large_binary() {
        ...
        assert!(
            ...
            "Large binary search took {:?}, expected <2000ms",
            duration
        );
    ...
    fn test_chain_building_performance() {
        ...
        assert!(
            ...
            "Chain building took {:?}, expected <1ms",
            duration
        );
    }
    ...
    fn test_pattern_search_performance() {
        ...
        assert!(
            duration.as_millis() < 100,
            "100 pattern searches took {:?}, expected <100ms",
            duration
        );
    }
}
...
mod advanced_tests {
    use super::*;
    use talon::rop_tools::{AutoROPSolver, Constraint, ROPGoal, ROPStrategy, RopChain};
    ...
    fn test_null_byte_constraint_enforcement() {
        ...
        if let Ok(solution) = result {
            for byte in &solution.chain_bytes {
                assert_ne!(
                    *byte, 0x00,
                    "Found null byte in chain with NoNullBytes constraint"
                );
            }
        }
    }
    ...
    fn test_complex_rop_chain() {
        ...
        let addresses = vec![0x400500, 0xdeadbeef, 0x400510, 0xcafebabe, 0x400520];
    ...
    fn test_gadget_quality_accuracy() {
        ...
        if rop.gadgets.len() > 1 {
            let syscall_gadgets: Vec<_> = rop
                .gadgets
                .iter()
                .filter(|g| g.instructions.iter().any(|i| i.contains("syscall")))
                .collect();

            let pop_gadgets: Vec<_> = rop
                .gadgets
                .iter()
            ...
            if !syscall_gadgets.is_empty() && !pop_gadgets.is_empty() {
                assert!(
                    syscall_gadgets[0].quality_score >= pop_gadgets[0].quality_score,
        ...
        for pattern in patterns {
            ...
            if !gadgets.is_empty() {
                for gadget in &gadgets {
                    ...
                    assert!(
                        gadget_str.contains(pattern),
                        "Gadget '{}' should contain pattern '{}'",
                        gadget_str,
                        pattern
                    );
                }
...
mod edge_case_tests {
    ...
    fn test_gadget_dedup_edge_cases() {
        let duplicate_code = vec![0x5f, 0xc3, 0x90, 0x5f, 0xc3, 0x90, 0x5f, 0xc3];
    ...
    fn test_high_address_ranges() {
        ...
        let high_addresses = vec![0x7ffff7a00000, 0x7ffff7b00000, 0x7ffff7c00000];
    ...
    fn test_boundary_conditions() {
        ...
        let boundary_addresses = vec![0x0, 0xFFFFFFFFFFFFFFFF, 0x8000000000000000];
