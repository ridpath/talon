...
fn test_house_of_spirit_next_chunk() {
    ...
    let next_chunk_size = u64::from_le_bytes(
        payload[fake_chunk_end + 8..fake_chunk_end + 16]
            .try_into()
            .unwrap(),
    );
...
fn test_tcache_to_malloc_hook_payload_structure() {
    ...
    let embedded_gadget = u64::from_le_bytes(
        chain[one_gadget_offset..one_gadget_offset + 8]
            .try_into()
            .unwrap(),
    );
...
fn test_glibc_version_from_string() {
    assert_eq!(
        GlibcVersion::from_string("2.23").unwrap(),
        GlibcVersion::V223
    );
    assert_eq!(
        GlibcVersion::from_string("2.27").unwrap(),
        GlibcVersion::V227
    );
    assert_eq!(
        GlibcVersion::from_string("2.31").unwrap(),
        GlibcVersion::V231
    );
    assert_eq!(
        GlibcVersion::from_string("2.32").unwrap(),
        GlibcVersion::V232
    );
    assert_eq!(
        GlibcVersion::from_string("2.35").unwrap(),
        GlibcVersion::V235
    );
    assert_eq!(
        GlibcVersion::from_string("2.39").unwrap(),
        GlibcVersion::V239
    );
}
...
fn test_solve_tcache_key_bypass_no_tcache_key() {
    ...
    assert!(result
        .unwrap_err()
        .contains("Tcache key validation not present"));
}
