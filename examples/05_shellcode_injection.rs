...
use talon::shellcode_db::{ShellcodeDatabase, get_shellcode};
use talon::shellcode_encoders::{xor_encode, alphanumeric_encode};
...
fn main() -> Result<(), String> {
    ...
    println!("\n[+] XOR-encoded (key=0x{:02x}): {} bytes", key, encoded.len());
    ...
    println!("    - Place shellcode at known address: 0x{:x}", shellcode_addr);
