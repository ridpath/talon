...
use talon::heap_grooming::{GroomingStrategy, HeapBlock, HeapGroom};
use talon::heap_tools::{HeapTarget, HeapTechnique};
...
fn example_manual_control() -> Result<(), String> {
    ...
    // Create context
    let _pwn = QuickPwn::remote("127.0.0.1", 9001, "./heap_vuln");
...
fn example_heap_grooming() -> Result<(), String> {
    ...
    let _holes = HeapGroom::new(
        "./vuln",
        GroomingStrategy::Holes {
            size: 0x90,
    ...
    let feng_shui = HeapGroom::new(
        "./vuln",
        GroomingStrategy::FengShui {
            layout: layout.clone(),
        },
    );
