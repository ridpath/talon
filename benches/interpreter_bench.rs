...
fn bench_control_flow(c: &mut Criterion) {
    ...
    group.bench_function("if_else", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(if_script)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
    ...
    group.bench_function("while_loop_100", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(while_script)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
    ...
    group.bench_function("for_loop_100", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(for_script)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
...
fn bench_function_calls(c: &mut Criterion) {
    ...
    group.bench_function("simple_function", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(simple_function)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
    ...
    group.bench_function("recursive_factorial_10", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(recursive_function)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
    ...
    group.bench_function("nested_calls", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(nested_calls)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
...
fn bench_builtin_functions(c: &mut Criterion) {
    ...
    let scripts = vec![
        ...
        ("p64_chain", "let chain = p64(0x1111) + p64(0x2222) + p64(0x3333)"),
        ("u64_unpack", r#"let value = u64("\x01\x02\x03\x04\x05\x06\x07\x08")"#),
...
fn bench_array_operations(c: &mut Criterion) {
    ...
    for size in [10, 100, 1000].iter() {
        group.bench_with_input(
            ...
            size,
            |b, &size| {
                let script = format!("let arr = [{}]", (0..size).map(|i| i.to_string()).collect::<Vec<_>>().join(", "));
...
fn bench_exploitation_primitives(c: &mut Criterion) {
    ...
    group.bench_function("rop_chain_construction", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(rop_chain)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
    ...
    group.bench_function("payload_building", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(payload_building)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
...
fn bench_full_exploit_scripts(c: &mut Criterion) {
    ...
    group.bench_function("buffer_overflow_exploit", |b| {
        b.iter(|| {
            let parser = TalonParser::new();
            let ast = parser.parse(black_box(buffer_overflow_exploit)).unwrap();
            let mut interpreter = Interpreter::new();
            black_box(interpreter.eval(&ast));
        });
    });
