...
fn bench_gadget_search(c: &mut Criterion) {
    ...
    for size in [1024, 4096, 16384, 65536].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(size), size, |b, &size| {
            ...
            b.iter(|| {
                let rop = RopChain::new(black_box(path)).unwrap();
                black_box(rop.gadgets.len());
            });
        });
    }
...
fn bench_chain_building(c: &mut Criterion) {
    ...
    for chain_length in [10, 50, 100, 500, 1000].iter() {
        group.bench_with_input(BenchmarkId::from_parameter(chain_length), chain_length, |b, &length| {
...
fn bench_gadget_finder(c: &mut Criterion) {
    ...
    let x64_code: Vec<u8> = (0..1024).map(|i| {
        if i % 16 == 0 { 0x5f }
        else if i % 16 == 1 { 0xc3 }
        else { 0x90 }
    }).collect();
    ...
    group.bench_function("analyze_bytes_1kb", |b| {
        b.iter(|| {
            ...
            finder.analyze_bytes(black_box(&x64_code), 0x400000).unwrap();
...
fn bench_gadget_quality_scoring(c: &mut Criterion) {
    ...
    c.bench_function("quality_scoring", |b| {
        b.iter(|| {
            let rop = RopChain::new(black_box(path)).unwrap();
            black_box(rop.gadgets.iter().map(|g| g.quality_score).sum::<u32>());
        });
    });
}
